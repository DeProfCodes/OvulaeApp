...
{
    public partial class EditPregnancyDatesModal : ContentView
    {
        private string CurrentSlide = "";

        private TaskCompletionSource<ModalCloseType> _resultCompletionSource;

        public EditPregnancyDatesModal()
        {
            InitializeComponent();
        }

        public Task<ModalCloseType> ShowPregnancyDateEditModal()
        {
            CurrentSlide = "DatesOptions";
            this.IsVisible = true;
            _resultCompletionSource = new TaskCompletionSource<ModalCloseType>();

            LastPeriodDate.SelectedDate = LocalStorageService.UserCycleProfile.LastPeriodDate.Value;
            DeliveryDate.SelectedDate = LocalStorageService.UserCycleProfile.LastPeriodDate.Value.AddDays(280);
            WeeksDue.Value = SharedCommonFunctions.GetCurrentCycleDayFromLMP(LocalStorageService.UserCycleProfile.LastPeriodDate.Value, LocalStorageService.UserCycleProfile.CycleLengthDays.Value);
