...
{
    public class PeriodLoggerViewModel : BaseViewModel
    {
        private DateTime _currentDate;
        private PeriodLogEntry _currentLogEntry;
        ...
        public PeriodLogEntry CurrentLogEntry
        {
        ...
        public PeriodLoggerViewModel(IModuleLogsService moduleLogsServ)
        {
        ...
        private void LoadLogForDate()
        {
            try
            {
                CurrentLogEntry = _moduleLogsServ.GetPeriodLogByDate(CurrentDate);
                if (CurrentLogEntry == null)
                {
                    CurrentLogEntry = DefaultValueHelper.CreateWithDefaults<PeriodLogEntry>();
        ...
        public async Task<bool> SaveCurrentLog()
        {
            try
            {
                var result = await _moduleLogsServ.UpdateTodayPeriodLog(CurrentLogEntry);
